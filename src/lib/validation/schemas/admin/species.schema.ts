import { z } from 'zod'

/**
 * Schéma de validation pour création/édition d'espèce
 *
 * ✅ RÈGLE #1 : Aucune valeur en dur
 * ✅ RÈGLE #6 : Messages i18n (clés, pas de texte)
 *
 * Tous les messages d'erreur sont des clés i18n qui seront traduites
 * dans le composant via `t(error.message)`
 *
 * @example
 * ```typescript
 * const result = speciesSchema.safeParse(data)
 * if (!result.success) {
 *   const errors = result.error.flatten().fieldErrors
 *   // errors.code = ['species.validation.code.required']
 * }
 * ```
 */
export const speciesSchema = z.object({
  /**
   * Code unique
   * - Requis
   * - Max 50 caractères
   * - Format : majuscules, chiffres, tirets, underscores
   */
  code: z.string()
    .min(1, 'species.validation.code.required')
    .max(50, 'species.validation.code.maxLength')
    .regex(
      /^[A-Z0-9_-]+$/,
      'species.validation.code.pattern'
    ),

  /**
   * Nom de l'espèce
   * - Requis
   * - Max 200 caractères
   */
  name: z.string()
    .min(1, 'species.validation.name.required')
    .max(200, 'species.validation.name.maxLength'),

  /**
   * Description (optionnel)
   * - Max 1000 caractères
   */
  description: z.string()
    .max(1000, 'species.validation.description.maxLength')
    .optional()
    .or(z.literal('')), // Accepte chaîne vide

  /**
   * Statut actif/inactif (optionnel)
   */
  isActive: z.boolean().optional(),
})

/**
 * Schéma pour mise à jour
 * Ajoute le champ `version` obligatoire pour optimistic locking
 */
export const updateSpeciesSchema = speciesSchema.extend({
  /**
   * Version pour optimistic locking
   * Doit être un entier positif
   */
  version: z.number()
    .int('species.validation.version.integer')
    .positive('species.validation.version.positive'),
})

/**
 * Type inféré pour les formulaires de création
 * Utilisé avec react-hook-form
 */
export type SpeciesFormData = z.infer<typeof speciesSchema>

/**
 * Type inféré pour les formulaires de mise à jour
 * Utilisé avec react-hook-form
 */
export type UpdateSpeciesFormData = z.infer<typeof updateSpeciesSchema>
